...
t_texture::t_texture(const unsigned int width, const unsigned int height,
                     const std::string name, GLenum internalformat,
                     GLenum format, void *data)
    : t_render_target(width, height, name), internalformat(internalformat),
      format(format) {
  glGenTextures(1, &this->id);
  glBindTexture(GL_TEXTURE_2D, this->id);
  glTexImage2D(GL_TEXTURE_2D, 0, internalformat, this->width, this->height, 0,
...
void t_texture::set(void *data) {
  ...
  glTexImage2D(GL_TEXTURE_2D, 0, this->internalformat, this->width,
               this->height, 0, this->format, GL_FLOAT, data);
  glGenerateMipmap(GL_TEXTURE_2D);
  glBindTexture(GL_TEXTURE_2D, 0);
}
