...
t_renderobject::t_renderobject(t_mesh *p_mesh, glm::mat4 transformation) {
    t_vertex_buffer *p_vertex_buffer = new t_vertex_buffer((float *) p_mesh->get_vertices()->data(), p_mesh->get_vertices()->size() * 8);
    t_vertex_element_buffer *p_vertex_element_buffer = new t_vertex_element_buffer(p_mesh->get_indices());
    this->p_vertex_buffer_instances = new t_vertex_buffer((float *) &transformation[0][0], 16);
    this->p_vertex_array = new t_vertex_array(p_vertex_buffer, p_vertex_element_buffer, this->p_vertex_buffer_instances);
    this->instances = 1;
}
...
void t_renderobject::draw() {
    ...
    glDrawElementsInstanced(GL_TRIANGLES, this->p_vertex_array->get_elements_count(), GL_UNSIGNED_INT, 0, this->instances);
...
void t_renderobject::update_instances(std::vector<glm::mat4> *transformations) {
    this->p_vertex_buffer_instances->update((float *) transformations->data(), transformations->size() * 16);
