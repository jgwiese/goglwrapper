...
t_texture_cubemap::t_texture_cubemap(std::string name,
                                     std::vector<std::string> paths)
    : t_render_target(2.0, 2.0, name) {
  assert(paths.size() == 6);
  ...
  for (unsigned int i = 0; i < paths.size(); i++) {
    ...
    glTexImage2D(GL_TEXTURE_CUBE_MAP_POSITIVE_X + i, 0, GL_RGBA,
                 p_image->get_width(), p_image->get_height(), 0, GL_RGBA,
                 GL_FLOAT, p_image->get_data());
